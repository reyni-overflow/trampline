<script lang="ts">
    ...
    function actionVariant(action: string): BadgeVariant {
        ...
        if (action.startsWith('reject') || action.startsWith('delete') || action === 'block_user')
            return 'error';
    ...
    {:else}
        <div class="audit-table">
            ...
            {#each filtered as entry (entry.id)}
                <div class="table-row">
                    <span class="col-date"
                        >{new Date(entry.createdAt).toLocaleDateString(getLocaleDateString(), {
                            day: 'numeric',
                            month: 'short',
                            year: 'numeric',
                            hour: '2-digit',
                    ...
                    <span class="col-action"
                        ><Badge variant={actionVariant(entry.action)} size="sm"
                            >{actionLabel(entry.action)}</Badge
                        ></span
                    >
                    ...
                    <span class="col-details" title={entry.details || ''}
                        >{entry.details || '—'}</span
                    >
