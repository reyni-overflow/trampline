    ...
    let apiPending = $state<
        {
            ...
            link: string | null;
            date: string;
            dadata: { found: boolean; fullName: string; kpp: string; loading: boolean };
        }[]
    >([]);
                ...
                <div class="verify-card">
                    <div class="card-header">
                        ...
                        <div class="card-info">
                            ...
                            <span class="card-date"
                                >{$t('adminVerif.applicationFrom')}
                                {new Date(company.date).toLocaleDateString(getLocaleDateString(), {
                                    day: 'numeric',
                                    month: 'short'
                                })}</span
                            >
                    ...
                    <div class="card-details">
                        <div class="detail-row">
                            <span class="detail-label">{$t('adminVerif.inn')}</span><span
                                >{company.inn}</span
                            >
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">{$t('adminVerif.email')}</span><span
                                >{company.email}</span
                            >
                        </div>
                        {#if company.link}<div class="detail-row">
                                <span class="detail-label">{$t('adminVerif.website')}</span><a
                                    href={company.link}
                                    target="_blank"
                                    rel="noopener">{company.link}</a
                                >
                            </div>{/if}
                    </div>
                    <div
                        class="dadata-result"
                        class:not-found={!company.dadata.loading && !company.dadata.found}
                    >
                        ...
                        {:else if company.dadata.found}
                            <span
                                >{company.dadata.fullName}
                                {company.dadata.kpp ? `(КПП: ${company.dadata.kpp})` : ''}</span
                            >
                        {:else}
                    ...
                    <div class="card-actions">
                        <Button onclick={() => approve(company.id, company.name)}
                            >{$t('common.approve')}</Button
                        >
                        <Button variant="danger" onclick={() => reject(company.id, company.name)}
