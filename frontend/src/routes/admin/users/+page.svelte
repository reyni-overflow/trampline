<script lang="ts">
    ...
    let apiUsers = $state<
        { id: string; name: string; email: string; role: string; date: string; blocked: boolean }[]
    >([]);
    ...
    <div class="users-table">
        ...
        {#each filtered as u (u.id)}
            <div class="table-row" class:blocked={u.blocked}>
                ...
                >
                <span class="col-date"
                    >{new Date(u.date).toLocaleDateString(getLocaleDateString(), {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric'
                    })}</span
                ...
                <div class="col-actions">
                    {#if u.role !== 'Admin'}
                        <Button
                            ...
                            title={$t('adminUsers.editProfile')}
                        >
                            <svg
                                viewBox="0 0 24 24"
                                width="14"
                        ...
                        {#if roleChangeTarget === u.id}
                            <div class="role-change-dropdown">
                                {#each changeRoleOptions as opt (opt.value)}
                                    {#if opt.value !== u.role}
                                        <button
                                            class="role-option"
                                            type="button"
                                            onclick={() => changeRole(u.id, u.name, opt.value)}
                                        >
                                            {opt.label}
                                        </button>
                                    {/if}
                                {/each}
                                <button
                                    class="role-option cancel"
                                    type="button"
                                    onclick={() => (roleChangeTarget = null)}>✕</button
                                >
                            </div>
                        {:else}
                            <Button
                        ...
                        {:else}
                            <Button
                                size="sm"
                                variant="ghost"
                                onclick={() => blockUser(u.id, u.name)}>{$t('common.block')}</Button
                            >
                        {/if}
                    {/if}
                </div>
...
<style>
    ...
    .controls :global(.select-group) {
        min-width: 10rem;
    }
    .controls :global(.select-trigger) {
        height: 2.5rem;
        font-size: var(--font-sm);
        padding: 0 0.75rem;
    }
    .controls :global(.option) {
        font-size: var(--font-sm);
    }
    .blocked-toggle {
        display: flex;
    ...
    .table-row {
        display: grid;
        grid-template-columns: 2fr 7rem 8rem 13rem;
        align-items: center;
