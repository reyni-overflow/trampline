                ...
                <div class="review-row">
                    <div class="review-main">
                        <div class="review-header">
                            ...
                            <Badge variant={review.isApproved ? 'success' : 'warning'} size="sm">
                                {review.isApproved ? $t('adminReviews.approved') : $t('adminReviews.pending')}
                            </Badge>
                        </div>
                    ...
                    <div class="review-actions">
                        {#if !review.isApproved}
                            <Button size="sm" onclick={() => approveReview(review.id)}>{$t('common.approve')}</Button>
                        {/if}
...
<style>
    ...
    .review-row:hover { border-color: var(--border-hover); }

    .review-main { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: var(--space-2); }

    .review-header { display: flex; align-items: center; gap: var(--space-3); }

    .review-author { display: flex; flex-direction: column; flex: 1; }
    .review-name { font-size: var(--font-sm); font-weight: var(--weight-semibold); }
    .review-role { font-size: var(--font-xs); color: var(--text-secondary); }

    .review-stars { font-size: var(--font-md); color: var(--color-warning, #f59e0b); letter-spacing: 0.1em; }
    ...
    .review-date { font-size: var(--font-xs); color: var(--text-tertiary); }

    .review-actions { display: flex; gap: var(--space-2); flex-shrink: 0; align-items: center; }
    ...
    .skeleton-row { min-height: 6rem; }
    .skeleton-avatar { width: 2.25rem; height: 2.25rem; border-radius: var(--radius-full); background: var(--bg-tertiary); animation: pulse 1.5s infinite; flex-shrink: 0; }
    .skeleton-lines { display: flex; flex-direction: column; gap: var(--space-2); flex: 1; }
    .skeleton-line { height: 0.75rem; border-radius: var(--radius-sm); background: var(--bg-tertiary); animation: pulse 1.5s infinite; }
    ...
    @media (max-width: 768px) {
        ...
        .review-actions { justify-content: flex-end; width: 100%; }
    }
</style>
