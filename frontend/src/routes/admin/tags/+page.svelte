<script lang="ts">
    ...
    let filtered = $derived(
        search ? allTags.filter((t) => t.name.toLowerCase().includes(search.toLowerCase())) : allTags
    );
...
<div class="tags-page">
    ...
    <div class="controls">
        <div class="search-box"><Input placeholder={$t('adminTags.searchPlaceholder')} bind:value={search} /></div>
        <form class="add-form" onsubmit={(e) => { e.preventDefault(); addTag(); }}>
            <div class="add-input-wrapper">
                <input class="add-input" type="text" placeholder={$t('adminTags.newTagPlaceholder')} bind:value={newTag} />
            ...
            <div class="tag-card">
                <div class="tag-main">
                    ...
                    {#if getStats(tag.name)}
                        <Badge variant="info" size="sm">{getStats(tag.name)?.eventCount ?? 0} {$t('adminTags.events')}</Badge>
                    {/if}
                </div>
                <button class="delete-btn" type="button" onclick={() => deleteTag(tag.name)} title={$t('common.delete')}>
...
<style>
    ...
    .add-input-wrapper:focus-within { border-color: var(--accent); box-shadow: 0 0 0 0.1875rem var(--accent-subtle); }
    .add-input-wrapper:hover:not(:focus-within) { border-color: var(--border-hover); }
    .add-input {
        flex: 1; height: 2.5rem; min-width: 0; padding: 0 0.75rem;
    ...
    .add-btn {
        ...
        width: 2rem; height: 2rem; flex-shrink: 0; margin-right: 0.25rem;
        color: var(--accent); border-radius: var(--radius-sm);
        transition: var(--transition-colors);
    }
    .add-btn:hover { background: var(--accent-subtle); }

    .tags-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(14rem, 1fr)); gap: var(--space-2); }
    .tag-card { display: flex; align-items: center; justify-content: space-between; padding: var(--space-3) var(--space-4); background: var(--bg-secondary); border: 1px solid var(--border-default); border-radius: var(--radius-md); }
    .tag-main { display: flex; align-items: center; gap: var(--space-2); }
    .delete-btn { display: flex; color: var(--text-tertiary); transition: var(--transition-colors); padding: var(--space-1); border-radius: var(--radius-sm); }
    .delete-btn:hover { color: var(--color-error); background: var(--color-error-subtle); }
</style>
