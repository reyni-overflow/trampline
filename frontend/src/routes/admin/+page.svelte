        ...
        try {
            const [users, verifications, jobs, events, companiesData, jobsData] = await Promise.all(
                [
                    ...
                    employeesApi.getAll(1, 1).catch(() => ({ items: [], totalCount: 0 })),
                    jobsApi.getAll(1, 1).catch(() => ({ items: [], totalCount: 0 }))
                ]
            );
            usersCount = String(users.length);
            companiesCount = String(companiesData.totalCount);
            jobsCount = String(jobsData.totalCount || 0);
            ...
            apiUsers = users.slice(0, 4).map((u) => ({
                name: u.nickname,
                role: u.role,
                date: new Date(u.createdAt).toLocaleDateString(getLocaleDateString(), {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric'
                })
        ...
        <div class="sections-row">
                    ...
                    {#each recentUsers as u (u.name)}
                        <div class="recent-row">
                            ...
                            <Badge
                                ...
                                size="sm"
                                >{u.role === 'Admin'
                                    ? $t('adminUsers.curator')
                                    : u.role === 'Employee'
                                      ? $t('adminDash.company')
                                      : $t('adminDash.worker')}</Badge
                            >
            ...
            <section class="dash-section">
                ...
                <div class="quick-actions">
                    <Button href="/admin/verification" variant="outline"
                        >{$t('adminDash.verificationCount', {
                            count: pendingVerifications
                        })}</Button
                    >
                    <Button href="/admin/moderation" variant="outline"
                        >{$t('adminDash.moderationCount', {
                            count: String(Number(pendingJobs) + Number(pendingEvents))
                        })}</Button
                    >
                    <Button href="/admin/users" variant="outline"
                        >{$t('adminDash.usersLink')}</Button
                    >
