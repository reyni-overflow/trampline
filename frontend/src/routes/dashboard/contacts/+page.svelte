<script lang="ts">
    ...
    onMount(async () => {
        try {
            ...
            contacts = contactsData.map(c => ({
                id: c.contactUserId,
                contactId: c.id,
                name: [c.lastName, c.name, c.patronymic].filter(Boolean).join(' ') || c.contactUserId,
                university: '',
                skills: c.skills || [],
                avatar: c.photo
            }));
    ...
    let filteredContacts = $derived(
        search ? contacts.filter((c) => c.name.toLowerCase().includes(search.toLowerCase())) : contacts
    );
    ...
    <div class="tab-content">
        {#if activeTab === 'contacts'}
            <div class="contacts-search">
                <SearchInput placeholder={$t('dashContacts.searchPlaceholder')} bind:value={search} />
            </div>
                    ...
                    {#each filteredContacts as contact (contact.id)}
                        <div class="contact-card">
                            ...
                            <div class="contact-actions">
                                <Button size="sm" variant="ghost" onclick={() => recommend(contact.id, contact.name)}>{$t('dashContacts.recommend')}</Button>
                                <Button size="sm" variant="ghost" onclick={() => removeContact(contact.contactId, contact.name)}>{$t('dashContacts.removeContact')}</Button>
                            </div>
                        </div>
                    {/each}
                </div>
        ...
        {:else if activeTab === 'incoming'}
            {#if incoming.length === 0}
                <div class="empty">
                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>
                    ...
                    {#each incoming as req (req.id)}
                        <div class="contact-card">
                            ...
                            <div class="request-actions">
                                <Button size="sm" onclick={() => acceptRequest(req.contactId, req.name)}>{$t('dashContacts.accept')}</Button>
                                <Button size="sm" variant="ghost" onclick={() => declineRequest(req.contactId, req.name)}>{$t('dashContacts.reject')}</Button>
                            </div>
                        </div>
                    {/each}
                </div>
        ...
        {:else if activeTab === 'recommendations'}
            {#if recommendations.length === 0}
                <div class="empty">
                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>
                    ...
                    {#each recommendations as rec (rec.id)}
                        <div class="contact-card rec-card">
                            <div class="rec-info">
                                ...
                                {/if}
                                <div class="rec-meta">
                                    <span class="rec-from">{$t('dashContacts.recommendedBy')} {rec.fromUserName}</span>
                                    <span class="rec-date">{new Date(rec.createdAt).toLocaleDateString()}</span>
                                </div>
                            </div>
                            <Button size="sm" href="/jobs/{rec.jobId}">{$t('dashContacts.viewJob')}</Button>
                        </div>
                    {/each}
                </div>
            {/if}
...
<style>
    ...
    .contacts-search { margin-bottom: var(--space-4); max-width: 20rem; }
    ...
    .contact-main:hover .contact-name { color: var(--accent); }
