<script lang="ts">
    ...
    function validateTitle() { errors.title = validate(title, [required]) || ''; }
    function validateDescription() { errors.description = validate(description, [required, minLength(20)]) || ''; }
    function validateCity() { errors.city = validate(city, [required]) || ''; }
    ...
    function validateAll(): boolean {
        validateTitle(); validateDescription(); validateCity(); validateAddress();
...
<div class="edit-mentorship">
    ...
    <div class="form-col">
        <Input label={$t('editMentorship.name')} bind:value={title} error={errors.title} onblur={validateTitle} oninput={() => clearError('title')} />
        <Select options={formatOptions} bind:value={format} label={$t('editMentorship.format')} />
        <MarkdownEditor label={$t('editMentorship.description')} bind:value={description} error={errors.description} onblur={validateDescription} oninput={() => clearError('description')} />
        <div class="form-row">
        ...
        {/if}
        <div class="form-row">
            <Input label={$t('createMentorship.maxParticipants')} type="number" bind:value={maxParticipants} />
            <div></div>
        </div>
        <div class="form-row">
            <Input label={$t('createMentorship.salaryFrom')} type="number" bind:value={salaryFrom} />
            <Input label={$t('createMentorship.salaryTo')} type="number" bind:value={salaryTo} />
        </div>
        <div class="form-row">
            <Input label={$t('editMentorship.city')} bind:value={city} error={errors.city} onblur={validateCity} oninput={() => clearError('city')} />
            <Input label={$t('editMentorship.country')} bind:value={country} />
        </div>
        {#if format !== 'Remote'}
            <Input label={$t('editMentorship.address')} bind:value={address} error={errors.address} onblur={validateAddress} oninput={() => clearError('address')} />
        {/if}
        ...
        <div class="form-actions">
            ...
            <Button size="lg" variant="outline" href="/dashboard/mentorships">{$t('editMentorship.cancel')}</Button>
        </div>
    </div>
</div>

<style>
    .edit-mentorship { display: flex; flex-direction: column; gap: var(--space-6); max-width: 40rem; }
    ...
    .tag-add { display: flex; gap: var(--space-2); align-items: flex-end; }
