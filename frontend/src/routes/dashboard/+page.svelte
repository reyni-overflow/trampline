<script lang="ts">
        ...
        try {
            if (role === 'Worker') {
                ...
                const accepted = apps.filter(
                    (a) =>
                        a.status === 'Invited' || a.status === 'Hired' || a.status === 'InProgress'
                ).length;
                workerStatsValues = {
                ...
                const [jobs, events, responseStats] = await Promise.all([
                ...
                employeeStatsValues = {
                    ...
                    responses: String(responseStats.totalResponses),
                    newOnes: String(responseStats.pendingResponses),
    ...
    let workerStats = $derived([
        {
            label: $t('dashOverview.applications'),
            value: workerStatsValues.applications,
            icon: '<path d="m22 2-7 20-4-9-9-4Z"/>'
        },
        {
            label: $t('dashOverview.pending'),
            value: workerStatsValues.pending,
            icon: '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>'
        },
        {
            label: $t('dashOverview.accepted'),
            value: workerStatsValues.accepted,
            icon: '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>'
        },
        {
            label: $t('dashOverview.favorites'),
            value: workerStatsValues.favorites,
    ...
    let employeeStats = $derived([
        {
            label: $t('dashOverview.jobs'),
            value: employeeStatsValues.jobs,
            icon: '<path d="M16 20V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/><rect width="20" height="14" x="2" y="6" rx="2"/>'
        },
        {
            label: $t('dashOverview.responses'),
            value: employeeStatsValues.responses,
            icon: '<path d="m22 2-7 20-4-9-9-4Z"/>'
        },
        {
            label: $t('dashOverview.newOnes'),
            value: employeeStatsValues.newOnes,
            icon: '<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/>'
        },
        {
            label: $t('dashOverview.views'),
            value: employeeStatsValues.views,
...
<div class="overview">
    <h1 class="overview-greeting">
        {role === 'Employee'
            ? $t('dashOverview.companyWelcome', { name: nickname })
            : $t('dashOverview.welcome', { name: nickname })}
    </h1>
                    ...
                    <a href="/jobs" class="onboarding-step">
                        <span class="ob-icon">
                            <svg
                                ...
                                stroke-width="1.75"
                                stroke-linecap="round"
                                stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4Z" /></svg
                            >
                        </span>
        ...
        {#if role === 'Worker'}
            <section class="overview-section">
                <div class="section-header">
                    ...
                    <a href="/dashboard/applications" class="section-link"
                        >{$t('dashOverview.allApps')}</a
                    >
                </div>
                <div class="applications-list">
        ...
        {:else}
            <section class="overview-section">
                ...
                <div class="quick-actions">
                    ...
                    <Button href="/dashboard/jobs" variant="outline"
                        >{$t('dashOverview.myJobs')}</Button
                    >
                    <Button href="/dashboard/profile" variant="ghost"
                        >{$t('dashOverview.editProfile')}</Button
                    >
                </div>
            </section>
        {/if}
