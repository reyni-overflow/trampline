    ...
    {:else}
        <div class="applications-table">
            <div class="table-header">
                <span class="col-job"
                    >{activeTab === 'events' ? $t('dashApps.event') : $t('dashApps.vacancy')}</span
                >
                <span class="col-company"
                    >{activeTab === 'events' ? $t('dashApps.city') : $t('dashApps.company')}</span
                >
                    ...
                    <a
                        href="{app.kind === 'event'
                            ...
                            : app.kind === 'mentorship'
                              ? '/mentorships'
                              : '/jobs'}/{app.jobId}"
                        class="col-job row-link">{app.jobTitle}</a
                    >
                    <span class="col-company">{app.company}</span>
                    <span class="col-date"
                        >{new Date(app.date).toLocaleDateString(getLocaleDateString(), {
                            day: 'numeric',
                            month: 'short'
                        })}</span
                    >
...
<style>
    ...
    .page-header :global(.select-trigger) {
        height: 2.25rem;
        font-size: var(--font-xs);
        padding: 0 0.75rem;
    }
    .page-header :global(.option) {
        font-size: var(--font-xs);
    }
