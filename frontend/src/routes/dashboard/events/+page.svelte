<script lang="ts">
    ...
    type EventItem = { id: string; title: string; format: string; city: string; views: number; createdAt: string; endedAt: string; status: string; isActive: boolean };
...
<div class="my-events">
    ...
    {:else if filtered.length === 0}
        <div class="empty">
            ...
            <p>{activeTab === 'active' ? $t('dashEvents.noActive') : activeTab === 'closed' ? $t('dashEvents.noClosed') : $t('dashEvents.noDrafts')}</p>
            <p>{activeTab === 'active' ? $t('dashEvents.noActiveHint') : activeTab === 'closed' ? $t('dashEvents.noClosedHint') : $t('dashEvents.noDraftsHint')}</p>
            {#if activeTab === 'active'}
                ...
                <div class="event-row">
                    <div class="event-info">
                        <div class="event-title-row">
                            ...
                            <Badge variant={event.format === 'Remote' ? 'success' : event.format === 'Office' ? 'warning' : 'default'} size="sm">{event.format === 'Remote' ? $t('dashJobs.remote') : event.format === 'Hybrid' ? $t('dashJobs.hybrid') : $t('dashJobs.office')}</Badge>
                        </div>
                        <div class="event-meta">
                            ...
                            {#if event.endedAt}
                                <span>&middot;</span>
                                <span>{$t('dashEvents.endsAt')} {new Date(event.endedAt).toLocaleDateString()}</span>
                            {/if}
                        </div>
                    </div>
                    <div class="event-stats">
                        <a href="/dashboard/events/{event.id}/responses" class="responses-link">
                            <Badge variant="accent">{event.views} {$t('dashEvents.responses')}</Badge>
                        </a>
                    </div>
                    <div class="event-actions">
...
<style>
    .my-events { display: flex; flex-direction: column; gap: var(--space-6); }
    ...
    .event-row:hover { border-color: var(--border-hover); }

    .event-info { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: var(--space-1); }
    .event-title-row { display: flex; align-items: center; gap: var(--space-2); flex-wrap: wrap; }
    .event-title { font-size: var(--font-sm); font-weight: var(--weight-semibold); transition: var(--transition-colors); }
    .event-title:hover { color: var(--accent); }
    .event-meta { display: flex; gap: var(--space-2); font-size: var(--font-xs); color: var(--text-tertiary); flex-wrap: wrap; }

    .event-stats { flex-shrink: 0; }
    .responses-link { text-decoration: none; }

    .event-actions { display: flex; gap: var(--space-1); flex-shrink: 0; }
