...
<div class="edit-event">
    ...
    <div class="form-col">
        <Input label={$t('editEvent.name')} bind:value={title} error={errors.title} onblur={validateTitle} oninput={() => clearError('title')} />
        ...
        <MarkdownEditor label={$t('editEvent.description')} bind:value={description} error={errors.description} onblur={validateDescription} oninput={() => clearError('description')} />
        <div class="form-row">
            <Input label={$t('editEvent.city')} bind:value={city} error={errors.city} onblur={validateCity} oninput={() => clearError('city')} />
            <Input label={$t('editEvent.country')} bind:value={country} />
        </div>
        {#if format !== 'Remote'}
            <Input label={$t('editEvent.address')} bind:value={address} error={errors.address} onblur={validateAddress} oninput={() => clearError('address')} />
        {/if}
        ...
        <div class="form-actions">
            ...
            <Button size="lg" variant="outline" href="/dashboard/events">{$t('editEvent.cancel')}</Button>
        </div>
    </div>
</div>

<style>
    .edit-event { display: flex; flex-direction: column; gap: var(--space-6); max-width: 40rem; }
