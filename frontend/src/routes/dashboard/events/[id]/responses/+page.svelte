<script lang="ts">
    ...
    import { eventsApi, type EventApplicationResponse } from '$lib/api/events';
    ...
    const STATUS_TO_BACKEND: Record<string, string> = {
        pending: 'Pending', accepted: 'Invited', rejected: 'Rejected', reserve: 'Viewed'
    };
    ...
    onMount(async () => {
        try {
            const data = await eventsApi.getResponses(page.params.id ?? '');
            responses = data.map((r: EventApplicationResponse) => ({
                id: r.id,
    ...
    let filtered = $derived.by(() => {
        ...
        if (search) {
            ...
            list = list.filter((r) => r.name.toLowerCase().includes(q) || r.skills.some((s) => s.toLowerCase().includes(q)));
        }
        return list;
    });
    ...
    async function changeStatus(id: string, status: string) {
        ...
        try {
            await eventsApi.updateApplicationStatus(id, backendStatus);
            responses = responses.map(r => r.id === id ? { ...r, status } : r);
            toast.success($t('responses.statusChanged', { status: statusMap[status]?.label || status }));
        } catch (err) {
            handleApiError(err);
        }
    }
...
<svelte:head>
    <title>{$t('eventResponses.pageTitle')}</title>
</svelte:head>
...
<div class="responses">
    ...
    <div class="page-header">
        <h1 class="page-heading">{$t('eventResponses.title')}</h1>
        <Badge variant="accent">{responses.length}</Badge>
    </div>

    <div class="controls-row">
        <div class="search-box">
            <Input placeholder={$t('eventResponses.searchPlaceholder')} bind:value={search} />
        </div>
        <Select options={statusOptions} bind:value={statusFilter} placeholder={$t('common.status')} />
    </div>

    {#if filtered.length === 0}
        <div class="empty"><p>{$t('eventResponses.noResponses')}</p></div>
    {:else}
            ...
            {#each filtered as resp (resp.id)}
                <div class="response-card">
                    ...
                    <div class="resp-actions">
                        <span class="resp-date">{new Date(resp.date).toLocaleDateString(getLocaleDateString(), { day: 'numeric', month: 'short' })}</span>
                        {#if resp.hasResume}
                            <Button size="sm" variant="ghost">{$t('eventResponses.resume')}</Button>
                        {/if}
                        <Select
...
<style>
    .responses { display: flex; flex-direction: column; gap: var(--space-6); }
    ...
    .controls-row { display: flex; gap: var(--space-3); align-items: flex-end; flex-wrap: wrap; }
    .search-box { width: 16rem; }
    .controls-row :global(.select-trigger) { height: 2.25rem; font-size: var(--font-xs); padding: 0 0.75rem; }
    .controls-row :global(.option) { font-size: var(--font-xs); }
    ...
    .resp-main { display: flex; align-items: center; gap: var(--space-4); flex: 1; min-width: 0; }
    .resp-user { display: flex; align-items: center; gap: var(--space-3); text-decoration: none; color: inherit; min-width: 12rem; }
    .resp-user:hover .resp-name { color: var(--accent); }
    .resp-user-info { display: flex; flex-direction: column; gap: 2px; }
    .resp-name { font-size: var(--font-sm); font-weight: var(--weight-semibold); transition: var(--transition-colors); }
    .resp-uni { font-size: var(--font-xs); color: var(--text-tertiary); }
    .resp-skills { display: flex; gap: var(--space-1); flex-wrap: wrap; }

    .resp-actions { display: flex; align-items: center; gap: var(--space-3); flex-shrink: 0; }
    .resp-actions :global(.select-trigger) { height: 2rem; font-size: var(--font-xs); padding: 0 0.5rem; min-width: 9rem; }
    .resp-actions :global(.option) { font-size: var(--font-xs); }
    .resp-date { font-size: var(--font-xs); color: var(--text-tertiary); }
    ...
    @media (max-width: 768px) {
        .response-card { flex-direction: column; align-items: stretch; }
        .resp-main { flex-direction: column; align-items: stretch; }
        .resp-user { min-width: 0; }
        .resp-actions { flex-wrap: wrap; }
    }
</style>
