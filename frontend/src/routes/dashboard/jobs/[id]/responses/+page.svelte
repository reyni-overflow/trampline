<script lang="ts">
    ...
    import { jobsApi, type JobApplicationResponse } from '$lib/api/jobs';
    ...
    onMount(async () => {
        try {
            const data = await jobsApi.getResponses(page.params.id ?? '');
            responses = data.map((r: JobApplicationResponse) => ({
                id: r.id,
...
<svelte:head>
    <title>{$t('jobResponses.pageTitle')}</title>
</svelte:head>
...
<div class="responses">
    ...
    <div class="page-header">
        <h1 class="page-heading">{$t('jobResponses.title')}</h1>
        <Badge variant="accent">{responses.length}</Badge>
    </div>

    <div class="controls-row">
        <div class="search-box">
            <Input placeholder={$t('jobResponses.searchPlaceholder')} bind:value={search} />
        </div>
        <Select options={statusOptions} bind:value={statusFilter} placeholder={$t('common.status')} />
    </div>

    {#if filtered.length === 0}
        <div class="empty"><p>{$t('jobResponses.noResponses')}</p></div>
    {:else}
            ...
            {#each filtered as resp (resp.id)}
                <div class="response-card">
                    ...
                    <div class="resp-actions">
                        ...
                        {#if resp.hasResume}
                            <Button size="sm" variant="ghost">{$t('jobResponses.resume')}</Button>
                        {/if}
                        <Select
