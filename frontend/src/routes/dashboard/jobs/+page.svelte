<script lang="ts">
    ...
    type JobItem = { id: string; title: string; type: string; format: string; city: string; salaryFrom: number | null; salaryTo: number | null; views: number; createdAt: string; status: string; isActive: boolean };
...
<div class="my-jobs">
    ...
    {:else if filtered.length === 0}
        <div class="empty">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><line x1="12" y1="12" x2="12" y2="12.01"/></svg>
            <p>{activeTab === 'active' ? $t('dashJobs.noActive') : activeTab === 'closed' ? $t('dashJobs.noClosed') : $t('dashJobs.noDrafts')}</p>
            <p>{activeTab === 'active' ? $t('dashJobs.noActiveHint') : activeTab === 'closed' ? $t('dashJobs.noClosedHint') : $t('dashJobs.noDraftsHint')}</p>
            {#if activeTab === 'active'}
                ...
                <div class="job-row">
                    <div class="job-info">
                        <div class="job-title-row">
                            ...
                            <Badge variant={job.type === 'Internship' ? 'info' : 'default'} size="sm">{job.type === 'Internship' ? $t('dashJobs.internship') : $t('dashJobs.vacancy')}</Badge>
                        </div>
                        <div class="job-meta">
                            <span>{job.city}</span>
                            <span>&middot;</span>
                            <span>{job.format === 'Remote' ? $t('dashJobs.remote') : job.format === 'Hybrid' ? $t('dashJobs.hybrid') : $t('dashJobs.office')}</span>
                            <span>&middot;</span>
...
<style>
    .my-jobs { display: flex; flex-direction: column; gap: var(--space-6); }
    ...
    .job-info { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: var(--space-1); }
    .job-title-row { display: flex; align-items: center; gap: var(--space-2); flex-wrap: wrap; }
    .job-title { font-size: var(--font-sm); font-weight: var(--weight-semibold); transition: var(--transition-colors); }
    .job-title:hover { color: var(--accent); }
    .job-meta { display: flex; gap: var(--space-2); font-size: var(--font-xs); color: var(--text-tertiary); flex-wrap: wrap; }

    .job-stats { flex-shrink: 0; }
    .responses-link { text-decoration: none; }

    .job-actions { display: flex; gap: var(--space-1); flex-shrink: 0; }
