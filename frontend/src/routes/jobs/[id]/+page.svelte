<script lang="ts">
    ...
    import { formatSalary, formatDate, jobTypeLabel, workFormatLabel, timeAgo, formatViews } from '$lib/utils/format';
    ...
    let data = $derived(job ?? {
        id: page.params.id ?? '', employeeId: '', userId: '', type: 'Work' as const, title: '...', description: '',
        address: '', city: '', country: '', street: '', geoLat: 0, geoLon: 0, salaryFrom: null, salaryTo: null,
        tags: [], format: 'Remote' as const, createdAt: '', updatedAt: '', deletedAt: null, endedAt: '',
            ...
            if (id) {
                ...
                try {
                    ...
                    similarJobs = allData.items
                        .filter(j => j.id !== id)
                        .map(j => ({ job: j, score: j.tags?.filter(t => currentTags.has(t.name)).length || 0 }))
    ...
    let typeBadge = $derived(
        data.type === 'Internship' ? 'info' as const :
        data.type === 'Mentorship' ? 'warning' as const :
        data.type === 'Event' ? 'accent' as const : 'default' as const
    );
    ...
    const jobJsonLd = $derived('<script type="application/ld+json">' + JSON.stringify({"@context":"https://schema.org","@type":"JobPosting","title":data.title,"description":data.description,"datePosted":data.createdAt,"jobLocation":{"@type":"Place","address":{"@type":"PostalAddress","addressLocality":data.city,"addressCountry":data.country}}}) + '</' + 'script>');
</script>
...
<div class="not-found container">
    <div class="not-found-card">
        <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="var(--text-tertiary)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>
    ...
    <div class="job-layout content-fade-in">
        <main class="job-main">
            <div class="job-header">
                <div class="job-header-top">
                    ...
                    <div class="header-actions">
                        ...
                        <button class="fav-btn" class:active={isFavorite} onclick={toggleFavorite} title={isFavorite ? $t('job.removeFromFavorites') : $t('job.addToFavorites')} type="button">
                ...
                <div class="job-title-row">
                    ...
                    <div class="job-title-info">
                        ...
                        <p class="job-location">{data.city}, {data.country} {#if data.address}&middot; {data.address}{/if}</p>
                    </div>
                </div>
            ...
            <div class="job-salary-bar">
                ...
                <span class="salary-value">{formatSalary(data.salaryFrom, data.salaryTo)}</span>
            </div>
            ...
            <div class="job-footer-info">
                ...
                {#if data.endedAt}
                    <span class="footer-item">&middot; {$t('job.deadline')} {formatDate(data.endedAt)}</span>
                {/if}
            </div>
        </main>
        ...
        <aside class="job-sidebar">
            {#if data.city}
                <div class="sidebar-card minimap-card">
                    ...
                    <div class="minimap-wrap">
                        <MapView
                            markers={[{ id: data.id, lat: data.geoLat || 55.757, lng: data.geoLon || 37.617, title: data.title, company: company?.name ?? '', type: data.type }]}
                            center={[data.geoLat || 55.757, data.geoLon || 37.617]}
                            zoom={14}
                            height="12rem"
                        />
                    </div>
                    <p class="minimap-address">{data.address ? `${data.address}, ` : ''}{data.city}, {data.country}</p>
                </div>
            {/if}
            ...
            <div class="sidebar-card">
                ...
                <div class="sidebar-company">
                    ...
                    <div>
                        <a href="/companies/{data.employeeId}" class="company-link">{company?.name ?? $t('job.company')}</a>
                ...
                {#if company?.info?.email}
                    <div class="company-contacts">
                        <a href="mailto:{company.info.email}" class="contact-link">
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.75"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
                            {company.info.email}
                        </a>
                    </div>
                {/if}
                {#if company?.link}
                    <a href={company.link} class="contact-link" target="_blank" rel="noopener">
                        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                ...
                <div class="similar-list">
                    {#each similarJobs as sj (sj.id)}
                        <a href="/jobs/{sj.id}" class="similar-item">
                            ...
                            <span class="similar-salary">{formatSalary(sj.salaryFrom, sj.salaryTo)}</span>
                            {#if sj.tags?.length}
                                <span class="similar-tags">{sj.tags.slice(0, 3).map(t => t.name).join(', ')}</span>
                            {/if}
                        </a>
                    {/each}
                </div>
...
{/if}
    ...
    <Modal open={applyModalOpen} title={$t('job.applyTitle')} onclose={() => applyModalOpen = false}>
        <div class="apply-modal-body">
            ...
            {/if}
            <span class="apply-counter" class:warn={coverLetter.trim().length < 50 || coverLetter.trim().length > 5000}>{coverLetter.trim().length} / 5000</span>
...
<style>
    ...
    @media (max-width: 1024px) {
        .job-skeleton { grid-template-columns: 1fr; }
    }
    .job-page {
