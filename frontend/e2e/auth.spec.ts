...
test.describe('Authentication', () => {
    test('login button opens auth modal', async ({ page }) => {
        ...
        const cookieAccept = page.locator('button').filter({ hasText: /ринять|ccept/i }).first();
        if (await cookieAccept.isVisible({ timeout: 2000 }).catch(() => false)) {
            await cookieAccept.click();
        }

        // Find and click login button in header
        const loginBtn = page.locator('header button, header a').filter({ hasText: /ойти|ogin|ход/i }).first();
    ...
    test('auth modal has role selection', async ({ page }) => {
        ...
        if (await loginBtn.count() > 0) {
            ...
            const body = await page.locator('.modal, .modal-overlay, [role="dialog"]').textContent();
    ...
    test('register button in hero section works', async ({ page }) => {
        ...
        const registerBtn = page.locator('.hero a, .hero button').filter({ hasText: /аботодатель|egister|mployer/i }).first();
